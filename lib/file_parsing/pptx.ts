...
export async function extractTextFromPPTX(pptxBuffer: Buffer): Promise<PPTXContent> {
  try {
    ...
    const slideFiles = directory.files
      .filter(
        (f: unzipper.File) => f.path.startsWith('ppt/slides/slide') && f.path.endsWith('.xml')
      )
      .sort((a, b) => {
    ...
    const allText = slides.map((slide, index) => `Slide #${index + 1}\n${slide}`).join('\n\n')
    console.log(allText)
...
export async function extractTextFromPPTXFile(file: File): Promise<PPTXContent> {
  try {
    ...
    throw new Error(
      `Failed to process PPTX file: ${error instanceof Error ? error.message : 'Unknown error'}`
    )
  }
}
